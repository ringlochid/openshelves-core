...
class Book(Base):
    ...
    tags: Mapped[list[str] | None] = mapped_column(ARRAY(String))  # Flat tag list: ['fantasy', 'classic']
...
class Review(Base):
    ...
    trust_awarded: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("0"))
    ...
    __table_args__ = (
        ...
        CheckConstraint("unhelpful_count >= 0", name="ck_reviews_unhelpful_count_positive"),
        CheckConstraint(
...
class CollectionBook(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("collection_id", "position", name="uq_collection_books_position"),
...
class PendingUpload(Base):
    ...
    # Upload Context
    upload_type: Mapped[str] = mapped_column(String(50), nullable=False)  # cover/avatar/file
    entity_type: Mapped[str | None] = mapped_column(String(50))  # author/book/collection
...
class AuthorFollow(Base):
    ...
    __table_args__ = (
        Index("ix_author_follows_author", "author_id", "created_at"),
    )
...
class BookSubscription(Base):
    ...
    __table_args__ = (
        Index("ix_book_subscriptions_book", "book_id", "created_at"),
    )
...
class CollectionSubscription(Base):
    ...
    # Relationships
    collection: Mapped["Collection"] = relationship("Collection", back_populates="subscribers")
