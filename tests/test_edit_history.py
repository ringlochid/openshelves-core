...
class TestChangeCalculation:
    ...
    def test_calculate_changes_complex(self):
        ...
        old = {"title": "Old Title", "year": 2020, "isbn": "123", "genre": "fiction"}
        new = {
            "title": "New Title",
            "year": 2020,
            "author": "John Doe",
...
class TestRecordEdit:
    ...
    async def test_record_create_convenience(self):
        ...
        history = await record_create(
            db=db, entity_type="author", entity_id=456, user_id=user_id, data=data
        )
...
# TestSerializeEntity tests removed - the new serialize_entity implementation uses
# Pydantic schemas (AuthorSerialize, BookSerialize, CollectionSerialize) which require
# real SQLAlchemy model instances, not MagicMocks. Serialization is verified by
# integration tests in test_collection_endpoints, test_book_update, etc.
