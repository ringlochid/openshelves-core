...
@pytest.mark.asyncio
async def test_owner_can_update_own_pending_author(
...
@pytest.mark.asyncio
async def test_owner_can_update_own_approved_author(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_owner_can_delete_own_author(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_non_owner_cannot_update_without_scope(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_wiki_editor_can_edit_approved_only(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_owner_without_delete_scope_fails(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_update_own_scope_only_works_on_owned(
    async_client: AsyncClient, test_db: AsyncSession
):
...
@pytest.mark.asyncio
async def test_permission_matrix_all_combinations(
    async_client: AsyncClient, test_db: AsyncSession
):
    ...
    test_cases = [
        ...
        (
            ...
            200,
            "Owner with edit_public_meta succeeds",
        ),
        ...
        (
            ...
            ["authors:edit_public_meta"],
            200,
            "Non-owner with wiki edit succeeds",
        ),
        (
            other_user_id,
            ["authors:update_own"],
            403,
            "Non-owner with only update_own fails",
        ),
