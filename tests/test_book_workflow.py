...
class TestBookApproval:
            ...
            with patch("routers.book.cache.invalidate_author", new=AsyncMock()):
                with patch("routers.book.cache.invalidate_book", new=AsyncMock()):
                    with patch(
                        ...
                        with patch(
                            "routers.book.adjust_trust_for_approval"
                        ) as mock_trust:
    ...
    async def test_approve_already_approved_fails(
        self, test_db, approved_book, curator_user
    ):
            ...
            with patch("routers.book.cache.invalidate_author", new=AsyncMock()):
                with patch("routers.book.cache.invalidate_book", new=AsyncMock()):
                    with patch(
                        ...
                        with patch(
                            "routers.book.adjust_trust_for_rejection"
                        ) as mock_trust:
                            mock_trust.return_value = AsyncMock()
                            ...
                            assert (
                                call_args["is_book"] is True
                            )  # Doubles penalty to -10

        assert book.status == ContentStatus.REJECTED
        assert book.is_public is False

    async def test_reject_already_rejected_fails(
        self, test_db, rejected_book, curator_user
    ):
...
class TestBookSoftDelete:
    ...
    async def test_delete_already_deleted_fails(
        self, test_db, deleted_book, curator_user
    ):
...
class TestBookSubscription:
    ...
    async def test_duplicate_subscription_fails(
        self, test_db, approved_book, regular_user
    ):
